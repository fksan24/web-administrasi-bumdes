      ...
      <div id="id00" class="w3-modal">
        <div class="w3-modal-content w3-card-4">
          ...
          <div class="w3-container">
            ...
            <a href="./edit-pembayaran-angsuran.php" class="w3-button w3-red">Tidak!</a>
          </div>
        </div>
      </div>
    </div>
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <!-- Index -->
  <div class="w3-container">
    <h5>Data Pembayaran Angsuran Edit</h5>
    <?php
        include '../koneksi/koneksi_mysql.php';
        $no_angsuran = $_GET['no_angsuran'];
        $tdpembayaranangsuran = mysqli_query($koneksi, "SELECT * FROM tdpembayaranangsuran WHERE no_angsuran='$no_angsuran'");
        while($tdpa = mysqli_fetch_array($tdpembayaranangsuran)){
    ?>
    <form method="post" action="update.php">
        <label for="no_angsuran">Nomor Angsuran :</label>
        <input class="w3-input w3-border" type="text" name="no_angsuran" size="13" maxlength="13" required value="<?php echo $tdpa['no_angsuran']; ?>" readonly>
        <label for="no_pinjaman">Nomor Pinjaman :</label>
        <select id="no_pinjaman" name="no_pinjaman" class="w3-input w3-border">
            <option><?php echo $tdpa['no_pinjaman']; ?></option>
            <option value=""></option>
            <option value="">- Pilih Kode Pinjaman Nasabah Lain -</option>
          <?php
        ...
        <input class="w3-input w3-border" type="number" name="angsuranke" min="1" max="12" required value="<?php echo $tdpa['angsuranke']; ?>">
        <label for="tgl_angsuran">Tanggal Angsuran :</label>
        <input class="w3-input w3-border" type="date" name="tgl_angsuran" required value="<?php echo $tdpa['tgl_angsuran']; ?>">
        <label for="jumlah_angsuran">Jumlah Angsuran :</label>
        <input class="w3-input w3-border" type="number" name="jumlah_angsuran" required value="<?php echo $tdpa['jumlah_angsuran']; ?>">
